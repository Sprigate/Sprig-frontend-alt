<footer
  class="footer sm:footer-horizontal bg-neutral text-neutral-content p-10 relative"
>
  <aside>
    <div class="flex flex-col gap-1">

    <!-- TODO: Ganti ke official svg icon. -->
      <h1 class="text-3xl font-bold">Sprig</h1>
      <p class="opacity-60 text-xs">
        &copy; 2026 Sprigate - All Rights Reserved.
      </p>
    </div>
  </aside>

  <!-- TODO: Isikan dengan menu aktual buat kedepannya -->
  <nav>
    <h6 class="footer-title">Contact</h6>
    <a class="link link-hover">Email</a>
    <a class="link link-hover">Github</a>
  </nav>
  <nav>
    <h6 class="footer-title">Sources</h6>
    <a class="link link-hover">DaisyUI</a>
    <a class="link link-hover">Svelte</a>
    <a class="link link-hover">Vite</a>
  </nav>
  <p class="absolute text-xs opacity-40 font-mono bottom-0 right-0 p-2">
    Sprig pre-alpha v1.2.0
  </p>
</footer>
